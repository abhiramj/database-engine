...
OrderMaker* CNF::MakeQOrderMaker(OrderMaker &sortedOn){
    OrderMaker *qMaker;
    OrderMaker cnfOrder;
    OrderMaker sortedOnCopy= sortedOn;
    GetSortOrders(cnfOrder,sortedOnCopy);

    qMaker= new OrderMaker();
    for (int i = 0; i < sortedOn.numAtts; i++)
    {
        bool isMatched = false;
        for(int j = 0; j < cnfOrder.numAtts; j++)
        {
            if((sortedOn.whichAtts[i] == cnfOrder.whichAtts[j]) && (sortedOn.whichTypes[i] == cnfOrder.whichTypes[j]))
            {
                // We init with 0 atts (for which atts) and set the current whichAtts(increments 1 per match) properly , then  set Type
                // After than we set numAtts (goes from 1 to numAtts)
                isMatched = true;
                qMaker->whichAtts[qMaker->numAtts] = qMaker->numAtts;
                qMaker->whichTypes[qMaker->numAtts] = sortedOn.whichTypes[i];
                qMaker->numAtts++;
                break;
            }
        }
