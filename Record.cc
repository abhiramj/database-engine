...
int Record :: GetNumAtts()
{
    ...
    int end = start + ((int*)bits)[1];
    numatt = (end-start)/start;
    return numatt;
}
...
void Record :: PrintToFile (FILE* outFile,Schema *mySchema,int recNo) {

    int n = mySchema->GetNumAtts();
    Attribute *atts = mySchema->GetAtts();
    if (!outFile){
        cout<<"Cannot open output file for printing"<<endl;
        exit(-1);
    }
    if (recNo==0){
        for (int i = 0; i < n; i++) {
