...
BigQ :: BigQ (Pipe &in, Pipe &out, OrderMaker &sortorder, int runlen) {
    ...
    //
    cout<<"Start"<<endl;
    //Write a function for doing this merge
    //Make a thread to run this query seperately. 
    inputPipe = &in;
    outputPipe= &out;
    SortOrder= &sortorder;
    //Record ss;
    ...
    lengthOfRun= runlen;
    pthread_t doSort;
    pthread_create(&doSort,NULL,&RunThread,(void *) this);
    //pthread_exit(NULL);
    //out.ShutDown();
}

int BigQ::WriteRun(int currPageOffset){
    ...
    int runIter=lengthOfRun;
    Record *rec,*copyRec;
    rec=new Record;
    int hasSpace=0;
    Page tempPage;
    int iterRec=0;
    while (runIter>0 && inputPipe->Remove(rec)==1)
    {
        iterRec++;
        copyRec=new Record;
        copyRec->Copy(rec);
...
int BigQ::ExternalSort() {

    int runlen=lengthOfRun;
    ...
    */
    ...
    do {
        ...
        outputPipe->Insert(recQ.top()->recStruct);
        recQ.pop();
